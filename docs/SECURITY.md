...
**Real client logs, screenshots and code must never be copied to a personal or development machine.**

Every control in this document — the sanitizer, data-layer authorization, retention, the decision not
to copy screenshots at all — assumes the data is inside the estate. A personal machine has none of
them, is not covered by the client contract, is not audited, and is not backed up under any policy
that applies here. Copying one failure folder "just to test with something realistic" undoes the lot.

Development uses a synthetic estate generated by `tools/make_fixtures.py`: realistic structure,
fabricated content, deterministic per seed. If it is not realistic enough, improve the generator.

**The one permitted exception**, because the parser cannot be written without it: a single log file,
sanitized by hand and reviewed before it leaves the estate, to establish the real format
(`OPEN_QUESTIONS.md` D1). One file, reviewed, once — not a workflow, and not a precedent for
